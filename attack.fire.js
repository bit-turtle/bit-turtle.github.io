...
function getScoreboard() {
	...
	ref[0].get().then((querySnapshot) => {
		querySnapshot.forEach((doc) => {
			...
			if(scoreboard[0][scoreboard[0].length-1].name.length>8){scoreboard[0][scoreboard[0].length-1].name=scoreboard[0].length-1].name.slice(0,7);scoreboard[0][scoreboard[0].length-1].name+="...";}
		});
